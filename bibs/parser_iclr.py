...
def parse_iclr(args):
    ...
    with open(os.path.join(args.save_dir, "{}.bib".format(args.full_name)), 'w') as f:
        for paper in tqdm(jar_paper):
            ...
            #2022, 2023
            if paper.find("a", class_="href_URL") is None:
                continue
            paper_url = paper.find("a", class_="href_URL")['href']
            
            if paper_url.startswith('https://jmlr'):
                continue
            #2021
            # paper_url = paper.find("a", class_="paper-pdf-link")['href']
            ...
            json_str = paper_data.find('script', {'type':'application/json'}).string
            data = json.loads(json_str)
            bibtex = data['props']['pageProps']['forumNote']['content']['_bibtex']
